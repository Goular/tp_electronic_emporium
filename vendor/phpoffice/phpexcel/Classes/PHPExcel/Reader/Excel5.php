...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	/**
	 * Mapping of XF index (that is a Styles XF) to final index in cellStyleXf collection
	 *
	 * @var array
	 */
	...
	public function load($pFilename)
	{
		...
		if (!$this->_readDataOnly) {
			$this->_phpExcel->removeCellStyleXfByIndex(0); // remove the default Styles
			$this->_phpExcel->removeCellXfByIndex(0); // remove the default Styles
		}
...
//		echo '<br />';
	 ...
	 * According to http://support.microsoft.com/kb/147732 there are always at least 15 cell Styles XF
	 * and 1 cell XF.
	 * Inspection of Excel files generated by MS Office Excel shows that XF records 0-14 are cell Styles XF
	 * and XF record 15 is a cell XF
	 * We only read the first cell Styles XF and skip the remaining cell Styles XF records
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// offset:  4; size: 2; XF type, cell protection, and parent Styles XF
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 14; size: 4;
					...
					// bit: 24-21; mask: 0x01E00000; diagonal Styles
				...
				// bit: 24-22; mask: 0x01C00000; bottom line Styles
			...
			if ($isCellStyleXf) {
				// we only read one Styles XF record which is always the first
				if ($this->_xfIndex == 0) {
				...
				switch ($extType) {
					case 4:		// fill start color
						...
						if ($xclfType == 2) {
							...
							// modify the relevant Styles property
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			// offset: 0; size: 2; index to XF record and flag for built-in Styles
			$ixfe = self::_GetInt2d($recordData, 0);

			// bit: 11-0; mask 0x0FFF; index to XF record
			$xfIndex = (0x0FFF & $ixfe) >> 0;

			// bit: 15; mask 0x8000; 0 = user-defined Styles, 1 = built-in Styles
			$isBuiltIn = (bool) ((0x8000 & $ixfe) >> 15);

			if ($isBuiltIn) {
				// offset: 2; size: 1; identifier for built-in Styles
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				for ($i = 0; $i <= $sstCount; ++$i) {
					...
					if (PHPExcel_Shared_String::CountCharacters($text) > 0) {
						if ($i == 0) { // first text run, no Styles
